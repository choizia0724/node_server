...
function parseKisTrade(raw: string): { code: string; price: number; volume: number } | null {
    if (!raw || raw[0] === "{") return null; // 제어/성공 JSON 스킵
    const segs = raw.split("|");
    if (segs.length < 4) return null;
    if (segs[1] !== "H0STCNT0") return null; // 체결 채널만 처리
    const F = segs[3].split("^");

    const code = F[0];
    const price = Number(F[3] ?? NaN); // 필요 시 인덱스 보정
    const volume = Number(F[9] ?? 0);  // 필요 시 인덱스 보정
    if (!code || !Number.isFinite(price)) return null;
    return { code, price, volume: Number.isFinite(volume) ? volume : 0 };
}

type Props = { code: string; initial: CandleDTO[] };
...
export default function StockChartLive({ code, initial }: Props) {
    ...
    useEffect(() => {
        ...
        ws.onmessage = (ev) => {
            const tick = parseKisTrade(String(ev.data ?? ""));
            ...
            setCandles((prev) => {
                ...
                const baseVol: number = Number((last as any).volume ?? 0);
                const volDelta: number = Number.isFinite(tick.volume) ? Number(tick.volume) : 0;
